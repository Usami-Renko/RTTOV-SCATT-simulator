'''
@Description: colorlib to handle issuses with colors
@Author: Hejun Xie
@Date: 2020-04-28 15:35:52
@LastEditors: Hejun Xie
@LastEditTime: 2020-04-28 16:11:48
'''
...
def get_hexcode(colorname):
    return cnames[colorname]

def get_hexcodes(colornames):
    return [get_hexcode(colorname) for colorname in colornames]

def gradient_color_names(color_list, color_sum=700):
    safe_pad = 5
    list = gradient_color(get_hexcodes(color_list), color_sum=color_sum+safe_pad)
    list = list[:color_sum]
    return list
...
if __name__ == '__main__':
    ...
    input_colors = ['red', 'yellow', 'green', 'blue', 'purple']
    colors = gradient_color_names(input_colors, color_sum=100)
    print(colors)
