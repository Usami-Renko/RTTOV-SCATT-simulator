PROGRAM rttovscatt_kad_padding_main
  ...
  ! my mod
  USE rttovscatt_mod, ONLY : plevels, FillValue, delta
...
#include "print_1d_data.interface"
  ...
  INTEGER(KIND=jpim), PARAMETER :: iooutJacobian  = 23    ! unit for output Jacobian profile
  ...
  real(KIND=jprb), dimension(:), allocatable    :: cfrac
  CHARACTER(LEN=32)                             :: shortName

  real(KIND=jprb), dimension(:), allocatable    :: temp_1d_data
  real(KIND=jprb)                               :: temp_data
  ...
  CHARACTER(LEN=256) :: jacobian_filename
  ...
  WRITE(*,*) 'enter path of JACOBIAN file'
  READ(*,*) jacobian_filename
...
!---------------[C]  levels field-----------------------
  ...
  ! --------------------------------------------------------------------------
  ! 7.1 Call RTTOV-SCATT forward model
  ! --------------------------------------------------------------------------
  IF (nthreads <= 1) THEN
    CALL rttov_scatt ( &
  ...
  ! --------------------------------------------------------------------------
  ! 7.2 Call RTTOV-SCATT Jacobian/Adjoint model
  ! --------------------------------------------------------------------------
  ...
  write(*,*) "start writing test_profile!"
...
!------------------------------------13----------------------------------------
  ...
  !====================================================
  !============== Jacobian results == start ===========

  write(*,*) "start writing Jacobian!"

write(*,*) "start writing Rain!"
OPEN(iooutJacobian, file=TRIM(jacobian_filename)//"_rain.dat", status='unknown', form='formatted', iostat=ios)
DO j = 1, nchannels
  DO l = 1, nlevels
    allocate(temp_1d_data(nprof))
    temp_1d_data = FillValue ! FillValue
    ! populate temp_1d_data
    DO iprof = 1, nprof
      joff = (iprof-1_jpim) * nchannels
      npad = npadlevels(iprof)
      IF (l+npad .gt. nlevels) THEN
        CYCLE
      ELSE
        temp_data = cld_profiles_kad(j+joff)%rain(l+npad)
        !IF (abs(temp_data) .gt. delta) temp_1d_data(iprof) = temp_data
        temp_1d_data(iprof) = temp_data  
      ENDIF
    ENDDO
    ! output 1d data
    CALL print_1d_data(temp_1d_data, iooutJacobian)
    deallocate(temp_1d_data)
  ENDDO
ENDDO
CLOSE(iooutJacobian, iostat=ios)

write(*,*) "start writing sp!"
OPEN(iooutJacobian, file=TRIM(jacobian_filename)//"_sp.dat", status='unknown', form='formatted', iostat=ios)
DO j = 1, nchannels
  DO l = 1, nlevels
    allocate(temp_1d_data(nprof))
    temp_1d_data = FillValue
    DO iprof = 1, nprof
      joff = (iprof-1_jpim) * nchannels
      npad = npadlevels(iprof)
      IF (l+npad .gt. nlevels) THEN
        CYCLE
      ELSE
        temp_data = cld_profiles_kad(j+joff)%sp(l+npad)
        !IF (abs(temp_data) .gt. delta) temp_1d_data(iprof) = temp_data
        temp_1d_data(iprof) = temp_data
      ENDIF
    ENDDO
...
write(*,*) "start writing clw!"
OPEN(iooutJacobian, file=TRIM(jacobian_filename)//"_clw.dat", status='unknown', form='formatted', iostat=ios)
DO j = 1, nchannels
  DO l = 1, nlevels
    allocate(temp_1d_data(nprof))
    temp_1d_data = FillValue
    DO iprof = 1, nprof
      ...
      IF (l+npad .gt. nlevels) THEN
        CYCLE
      ELSE
        temp_data = cld_profiles_kad(j+joff)%clw(l+npad)
        !IF (abs(temp_data) .gt. delta) temp_1d_data(iprof) = temp_data
        temp_1d_data(iprof) = temp_data  
      ENDIF
    ENDDO
...
write(*,*) "start writing ciw!"
OPEN(iooutJacobian, file=TRIM(jacobian_filename)//"_ciw.dat", status='unknown', form='formatted', iostat=ios)
DO j = 1, nchannels
  DO l = 1, nlevels
    allocate(temp_1d_data(nprof))
    temp_1d_data = FillValue
    DO iprof = 1, nprof
      ...
      IF (l+npad .gt. nlevels) THEN
        CYCLE
      ELSE
        temp_data = cld_profiles_kad(j+joff)%ciw(l+npad)
        !IF (abs(temp_data) .gt. delta) temp_1d_data(iprof) = temp_data
        temp_1d_data(iprof) = temp_data 
      ENDIF
    ENDDO
...
write(*,*) "start writing cc!"
OPEN(iooutJacobian, file=TRIM(jacobian_filename)//"_cc.dat", status='unknown', form='formatted', iostat=ios)
DO j = 1, nchannels
  DO l = 1, nlevels
    allocate(temp_1d_data(nprof))
    temp_1d_data = FillValue
    DO iprof = 1, nprof
      ...
      IF (l+npad .gt. nlevels) THEN
        CYCLE
      ELSE
        temp_data = cld_profiles_kad(j+joff)%cc(l+npad)
        temp_1d_data(iprof) = temp_data ! zero cc is acceptable  
      ENDIF
    ENDDO
    CALL print_1d_data(temp_1d_data, iooutJacobian)
    deallocate(temp_1d_data)
...
write(*,*) "start writing cfrac!"
OPEN(iooutJacobian, file=TRIM(jacobian_filename)//"_cfrac.dat", status='unknown', form='formatted', iostat=ios)
allocate(temp_1d_data(nprof))
temp_1d_data = FillValue
temp_1d_data = cfrac
CALL print_1d_data(temp_1d_data, iooutJacobian)
...
END PROGRAM rttovscatt_kad_padding_main
