...
Fig. \ref{fig:MWRI1ABSP44} shows the bulk scattering properties in ice-phase layers accounting for all the hydrometeor types and also the
gas absorption. The vertical gradient of temperature plays an significant role in this figure. For instance, note that the boundary layer is near 
...
for both shapes as a consequence of the temperature dependency of snow BSP.
...
\caption{10.65GHZ H bulk scattering properties accounting for both hydrometeors and gas absorption for ice-phase layers.
...
\caption{18.7GHZ H bulk scattering properties accounting for both hydrometeors and gas absorption for ice-phase layers.
$Factor_{upper} = Factor_{lower} = 10^{1}$.}
...
the process when mixing ration of hydrometeor of lower layer is extremely high.   
    The above mechanism accounts for the abnormal pattern at top right corner of the figure.
...
\oops{Generalization for shielding effect}: From the above analysis, we find that the habit layer with higher extiction and SSA tends to shield the mixing ratio of another habit layer, no matter
