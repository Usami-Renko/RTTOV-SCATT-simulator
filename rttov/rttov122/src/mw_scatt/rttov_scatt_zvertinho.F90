...
!
Subroutine rttov_scatt_zvertinho(   &
      & errorstatus,          &! out
      & opts_scatt,           &! in
      & nlevels,              &! in
      & chanprof,             &! in
...
!* Subroutine arguments:
  ...
  Real (Kind=jprb),       optional, Intent (in)    :: gh(size(profiles), nlevels)          ! Geopotential height, present if use z-coord modelvar [km]
!INTF_END

#include "rttov_direct.interface"
#include "rttov_iniscatt_zvertinho.interface"
...
#include "rttov_scatt_emis_terms.interface"
  ...
  Real (Kind=jprb), target :: t__tau_levels (nlevels+1,size(chanprof))
...
!* Local variables:
  ...
  if (lhook) call dr_hook('RTTOV_SCATT_ZVERTINHO',0_jpim,zhook_handle)
  ...
  !    IF (LHOOK) CALL DR_HOOK('RTTOV_SCATT_ZVERTINHO',1_jpim,ZHOOK_HANDLE)
  !    Return
  !  End If
  !End do
  ...
  write(*,*) 'direct_nlevels:', profiles(1) % nlevels
  ...
  WRITE(*,*) "before enter rttov_iniscatt_zvertinho"
  !---------------------------- Z coord -----------------------------
  If(present(gh)) then
    Call rttov_iniscatt_zvertinho(   &
           & errorstatus,           &! out
           ...
           & scatt_aux,             &! inout
           & gh)                     ! in
    WRITE(*,*) "after enter rttov_iniscatt_zvertinho"

    If ( errorstatus == errorstatus_fatal ) Then
       Write( errMessage, '( "error in rttov_iniscatt_zvertinho")' )
       Call rttov_errorreport (errorstatus_fatal, errMessage, NameOfRoutine)
       IF (LHOOK) CALL DR_HOOK('RTTOV_SCATT_ZVERTINHO',1_jpim,ZHOOK_HANDLE)
       Return
    End If
  ...
  Else
    ...
    If ( errorstatus == errorstatus_fatal ) Then
       ...
       Call rttov_errorreport (errorstatus_fatal, errMessage, NameOfRoutine)
       IF (LHOOK) CALL DR_HOOK('RTTOV_SCATT_VERTINHO',1_jpim,ZHOOK_HANDLE)
       Return
    End If  
  EndIf
...
End Subroutine rttov_scatt_zvertinho
